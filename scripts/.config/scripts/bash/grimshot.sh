...
ACTION=${1:-usage}
SUBJECT=${2:-screen}
FILE=${3:-$(getTargetDirectory)/$(date '+%d-%m-%Y@%I:%M:%S%p').png}
...
notify() {
	notify-send -a grimshot "$1" "$2" -i $3 -c screenshot
}
...
if [ "$ACTION" = "copy" ] ; then
	...
	tmp=$(takeScreenshot - "$GEOM" "$OUTPUT" | base64)
	image=$(takeScreenshot "$FILE" "$GEOM" "$OUTPUT" | base64)
	if true; then
		echo "$tmp" | base64 --decode > /tmp/tmp.png
		TITLE="Screenshot of $SUBJECT"
		notifyOk "$FILE" "$TITLE" "/tmp/tmp.png"
		echo $FILE
	else
